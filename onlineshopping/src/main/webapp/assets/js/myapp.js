...
/*
 * case 'All Products': $('#listProducts').addClass('active'); break; case
 * 'Product Management': $('#manageProduct').addClass('active'); break; case
 * 'Shopping Cart': $('#userModel').addClass('active'); break; default: if (menu ==
 * "Home") break; $('#listProducts').addClass('active'); $('#a_' +
 * menu).addClass('active'); break;
 * 
 ...
 * '/json/data/all/products'; } else { jsonUrl = window.contextRoot +
 * '/json/data/category/' + window.categoryId + '/products'; }
 * 
 * $table .DataTable({
 * 
 * lengthMenu : [ [ 3, 5, 10, -1 ], [ '3 Records', '5 Records', '10 Records',
 * 'ALL' ] ], pageLength : 5, ajax : { url : jsonUrl, dataSrc : '' }, columns : [ {
 ...
 * console.log(jsonUrl);
 * 
 * $productsTable.DataTable({ lengthMenu : [ [ 10, 30, 50, -1 ], [ '10 Records',
 * '30 Records', '50 Records', 'ALL' ] ], pageLength : 30, ajax : { url :
 * jsonUrl, dataSrc : '' }, columns : [ {data: 'id'},
 * 
 * 
 * {data: 'code', bSortable: false, mRender: function(data,type,row) { return '<img
 * src="' + window.contextRoot + '/resources/images/' + data + '.jpg"
 * class="dataTableImg"/>'; } }, { data : 'name' }, { data : 'brand' }, { data :
 ...
 * 
 * }else { str += '<label class="switch"> <input type="checkbox"
 * value="'+row.id+'"> <div class="slider round"> </div></label>'; }
 * 
 * return str; } }, { data : 'id', bSortable : false, mRender : function(data,
 * type, row) {
 * 
 * var str = ''; str += '<a href="' + window.contextRoot + '/manage/' + data +
 ...
 * (this.checked)? 'You want to activate the Product?': 'You want to de-activate
 * the Product?'; var checked = this.checked; var checkbox = $(this); debugger;
 * bootbox.confirm({ size: 'medium', title: 'Product Activation/Deactivation',
 * message: dText, callback: function (confirmed) { if (confirmed) { $.ajax({
 * type: 'GET', url: window.contextRoot +
 ...
 * 
 * $categoryForm.validate({ rules: { name: { required: true, minlength: 3 },
 * description: { required: true, minlength: 3 } }, messages: { name: {
 ...
 * 
 * 
 * ------ for fading out the alert message after 3 seconds $alert = $('.alert');
 * if($alert.length) { setTimeout(function() { $alert.fadeOut('slow'); }, 3000 ); }
 * 
 * ------ handle refresh cart $('button[name="refreshCart"]').click(function(){
 * var cartLineId = $(this).attr('value'); var countField = $('#count_' +
 * cartLineId); var originalCount = countField.attr('value'); // do the checking
 * only the count has changed if(countField.val() !== originalCount) { // check
 * if the quantity is within the specified range if(countField.val() < 1 ||
 * countField.val() > 3) { // set the field back to the original field
 * countField.val(originalCount); bootbox.alert({ size: 'medium', title:
 * 'Error', message: 'Product Count should be minimum 1 and maximum 3!' }); }
 * else { // use the window.location.href property to send the request to the
 * server var updateUrl = window.contextRoot + '/cart/' + cartLineId +
 * '/update?count=' + countField.val(); window.location.href = updateUrl; } }
 * }); });
 */